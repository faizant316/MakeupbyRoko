...
// Public endpoint — no admin auth required (used by the bridal inquiry form)
export async function POST(req) {
  try {
    ...
    const path = `inspo/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
    ...
    console.error('upload-inspo-photo:', err);
