    ...
    <div className="w-full mb-12 pt-7" style={{ borderTop: `1px solid ${dm ? '#2e2e38' : '#f1ece7'}` }}>
      ...
      <button onClick={onOpenAll} className="group w-full flex items-center gap-3 mb-4 text-left">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(160,96,122,0.1)' }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="#A0607A" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-[17px] h-[17px]">
      ...
      {/* Status breakdown — minimal inline (dot · count · label) */}
      <div className="flex items-center gap-7 sm:gap-9 pl-0.5">
      ...
      {recent.length > 0 && (
        <div className="mt-5">
          ...
          <div className="flex flex-col gap-1">
            {recent.slice(0, 3).map(r => {
              ...
              return (
                <button key={r.id} onClick={() => onOpenReg?.(r)}
                  ...
                  style={{ background: dm ? '#1e1e24' : '#FAF7F8' }}
                  onMouseEnter={e => { e.currentTarget.style.background = dm ? '#2a242a' : '#F4EBF0'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = dm ? '#1e1e24' : '#FAF7F8'; }}>
            ...
            {recent.length > 3 && (
              <button onClick={onOpenAll} className="text-[0.65rem] font-semibold py-1 pl-0.5 text-left transition-colors hover:opacity-70" style={{ color: dm ? '#c78fa3' : '#A0607A' }}>
