...
// The one month grid used by every admin calendar: the full Calendar tab, the
// All Appointments overlay, and the compact Home picker (dense). Keeping a
// single grid is the point — three different-looking calendars for the same
// data was the confusing part.
//
...
// "0 of 4 booked" in plain words, plus what else is on the day so a day with
// consultations never reads as empty.
export function daySummary(events, booked, cap) {
  const parts = [];
  if (cap != null) parts.push(`${booked} of ${cap} booking${cap === 1 ? '' : 's'}`);
...
function Checkbox({ on, dm }) {
  return (
    <span className="w-[18px] h-[18px] rounded-md flex items-center justify-center flex-shrink-0 transition-all"
      ...
      {on && (
        <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round" className="w-2.5 h-2.5">
...
export default function MonthCalendar({
  ...
  const minHeight = dense ? 74 : 104;

  return (
    <>
      <div className="grid grid-cols-7 gap-1.5 sm:gap-2 mb-2">
        {DAYS.map(d => (
          <div key={d} className="text-center text-[0.62rem] font-semibold py-1 uppercase tracking-widest"
            style={{ color: dm ? '#71717a' : '#bbb' }}>{d}</div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
        {cells.map((day, idx) => {
          if (day === null) {
            return <div key={`e-${idx}`} className="rounded-xl" style={{ background: dm ? 'rgba(255,255,255,0.015)' : '#FBFBFD' }} />;
          }
          const key = `${monthPrefix}-${pad(day)}`;
          const events = evMap[key] || [];
          const isToday = key === todayKey;
          const off = offMap[key] != null;
          ...
          return (
            <div
              ...
              role="button"
              tabIndex={0}
              onClick={() => (selectMode ? onToggleDay?.(key) : onOpenDay?.(key))}
              onKeyDown={(e) => {
              ...
              title={selectMode
                ...
                : `${daySummary(events, booked, cap) || 'Nothing scheduled'}${off ? ' · Day off' : ''}`}
            ...
            >
              <div className="flex items-center justify-between gap-1 px-0.5">
                ...
                {/* Counts EVERY item on the day, so it never disagrees with the
                    booking line below. */}
                {events.length > 0 && (
                  <span className="text-[0.58rem] font-semibold tabular-nums px-1.5 py-0.5 rounded-full flex-shrink-0"
              ...
              {events.slice(0, maxChips).map(ev => {
                ...
                return (
                  <div
                    ...
                    onClick={(e) => { if (!onEventClick || selectMode) return; e.stopPropagation(); onEventClick(ev); }}
                    onKeyDown={(e) => {
              ...
              {cap != null && !off && (
                <span className="mt-auto pl-0.5 text-[0.56rem] font-semibold tabular-nums tracking-wide"
                  style={{ color: full ? '#E0795B' : booked > 0 ? (dm ? '#a1a1aa' : '#83838d') : (dm ? '#52525b' : '#c2c2cb') }}>
                  {full ? 'Fully booked' : `${booked}/${cap} booked`}
                </span>
              )}
            </div>
          );
