      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3" style={{ alignItems: 'start' }}>
          {registrations.map(reg => {
            ...
            return (
              <div key={reg.id} style={isExpanded ? { gridColumn: '1 / -1' } : {}}>
                {/* Main card */}
                <button
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-2.5">
                    <h4 className="font-serif text-[1rem] group-hover:text-[#D4A0B0] transition-colors" style={{ color: dm ? '#F0EBE6' : '#111' }}>
                  ...
                  <div className="flex flex-col gap-1 mb-2.5">
                    <div className="flex items-center gap-2 text-[0.72rem]"
                      style={{ color: appointmentDate ? (dm ? '#71717a' : '#999') : (dm ? '#52525b' : '#ccc') }}>
                      ...
                      {appointmentDate ? `${appointmentDate}${reg.appointment_time ? ` at ${reg.appointment_time}` : ''}` : 'No date scheduled yet'}
                    </div>
                    {reg.email && (
                      <div className="flex items-center gap-2 text-[0.72rem]" style={{ color: dm ? '#71717a' : '#999' }}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3.5 h-3.5 flex-shrink-0">
                  ...
                  <p className="text-[0.72rem] font-semibold mb-2.5" style={{ color: dm ? '#D4A0B0' : '#A0785A' }}>
                    ...
                    {depositAmount > 0 && <span className="ml-1.5 font-normal text-[0.67rem]" style={{ color: dm ? '#71717a' : '#bbb' }}>(${depositAmount} deposit)</span>}
                  </p>

                  {/* Footer: payment badge + contact buttons */}
                  <div className="flex items-center gap-2 pt-2.5" style={{ borderTop: `1px solid ${dm ? '#3a3a48' : '#f0ebe6'}` }}>
                    <span className="text-[0.58rem] font-semibold tracking-[0.08em] uppercase px-2.5 py-0.5 rounded-full"
                ...
                {/* Expanded detail panel — full width (grid-column already set on parent) */}
                {isExpanded && (
                  <div
