...
export function consultationScheduledEmail({ firstName, serviceName, consultationDate, consultationTime, consultationType, zoomLink, consultationNotes }) {
  ...
  const gcalDetails = encodeURIComponent(`Consultation with Roqia Moshref (Makeup by Roko) for ${serviceName}${zoomLink ? `\nZoom: ${zoomLink}` : ''}`);
  const gcalUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${gcalTitle}&details=${gcalDetails}`;

  return base(`
    ${card(`
      <div style="text-align:center;margin-bottom:14px;">
        <div style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#F2EDF1,#E5D5E1);margin:0 auto 12px;line-height:48px;text-align:center;font-size:22px;">📅</div>
        <h1 style="font-family:Georgia,serif;font-size:22px;font-weight:300;color:#2C1A14;margin:0 0 4px;">Consultation <em style="color:#AF90A9;">Scheduled!</em></h1>
    ...
    `)}
    <div style="background:#fff;border-radius:14px;padding:18px;margin-bottom:10px;border:2px solid #D4BECD;">
      <p style="font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#AF90A9;margin:0 0 10px;">Consultation Details</p>
      <table style="width:100%;border-collapse:collapse;">
      ...
      ${zoomLink ? `
      <div style="margin-top:14px;padding:12px;background:#F7F2F6;border-radius:10px;border:1px solid #D4BECD;">
        <p style="font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#AF90A9;margin:0 0 8px;">Your Zoom Link</p>
        <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
          <a href="${zoomLink}" style="display:inline-block;background:#AF90A9;color:#fff;padding:12px 24px;border-radius:10px;font-size:14px;font-weight:700;text-decoration:none;">💻 Join Zoom Call</a>
        </td></tr></table>
        <p style="font-size:11px;color:#9E8E84;text-align:center;margin:8px 0 0;word-break:break-all;">${zoomLink}</p>
      </div>` : ''}
      <div style="margin-top:14px;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
          <a href="${gcalUrl}" style="display:inline-block;background:#F7F2F6;color:#AF90A9;padding:10px 22px;border-radius:10px;font-size:13px;font-weight:700;text-decoration:none;border:1px solid #D4BECD;">+ Add to Google Calendar</a>
        </td></tr></table>
      </div>
    </div>
    ${card(`
  ...
  return base(`
    ...
    <div style="background:#fff;border-radius:14px;padding:18px;margin-bottom:10px;border:1px solid #EDE6DF;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        ${zoomLink ? row('Zoom Link', `<a href="${zoomLink}" style="color:#AF90A9;word-break:break-all;">${zoomLink}</a>`) : ''}
        ${consultationNotes ? row('Notes', consultationNotes) : ''}
      </table>
    </div>
    ${card(`
